...
!! Question: reversible or irreversible ? avec nadph or not ?
!! j=Kw avec K(nr,nm) et w(nm); Cj(nr,nr)=K(nr,nm) Cw(nm,nr)
!! dj(nr,nr)-1 Cj(nr,nr) dj(nr,nr) = dj(nr,nr)-1 K(nr,nm) dw(nm,nm) Cw(nm,nr); C_j*=K*C_w
!! GPI larger and g6pd smaller
!! 32large,37small,38large,49pgsmall
module GLOB
  implicit none
...
program principal !!REF
  ...
  open(13,file='wp_cc1.res');    !whiskecrplot for cs OUI 
  open(73,file='wp_c.res');      !whiskecrplot for cs
  open(173,file='wp_cx.res');    !whiskecrplot for cs
  open(79,file='wp_j.res');        !whiskerplot for cj only g6pd SORTIE OUI
  open(179,file='wp_jx.res');      !minimum and maximu (grey on figure) OUI
  open(21,file='all.res');
  open(30,file='param504208.dat'); open(31,file='param851005.dat');
  open(32,file='param108505.dat'); open(33,file='param473817.dat'); 
  open(34,file='param601030.dat'); open(35,file='param106030.dat'); 
...
! to get PG increase when knad increase (correlated with PG increase for any reaction increase!!)
! need nadph large near
! need pg inhibit GPI 0.1 at least
